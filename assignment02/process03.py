...
class Basket:
    ...
    def use_eggs(self, self_index):
        print(f"{ctime()} Kitchen–{self_index} : Chef–{self_index} has lock with eggs remaining {self.eggs.value}")
        self.eggs.value -= 1
        print(f"{ctime()} Kitchen–{self_index} : Chef–{self_index} has released lock with eggs remaining {self.eggs.value}")
